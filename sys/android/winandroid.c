#include <string.h>
static int nhcolor_to_RGB(int c);
#include <errno.h>
#include <jni.h>
...
{
	debuglog("and_curs wid=%d %dx%d", wid, x, y);
	
	// Clamp cursor coordinates to prevent Java array access violations
	int clamped_x = x;
	int clamped_y = y;
	
	if (clamped_x < 0) clamped_x = 0;
	if (clamped_x >= 80) clamped_x = 79;
	if (clamped_y < 0) clamped_y = 0;
	if (clamped_y >= 21) clamped_y = 20;
	
	if (x != clamped_x || y != clamped_y) {
		debuglog("CURSOR CLAMPED from %dx%d to %dx%d", x, y, clamped_x, clamped_y);
	}
	
	JNICallV(jSetCursorPos, wid, clamped_x, clamped_y);
}
...
void and_add_menu(winid wid, int glyph, int cnt, const ANY_P *ident, CHAR_P accelerator, CHAR_P groupacc, int attr, const char *str, BOOLEAN_P preselected)
{
	...
	jbyteArray jstr = create_bytearray(str);
	JNICallV(jAddMenu, wid, tile, ident->a_long, (int)accelerator, (int)groupacc, 1<<attr, jstr, (int)preselected, color);
	destroy_jobject(jstr);
}
...
{
	debuglog("and_print_glyph wid=%d %dx%d glyph=%d", wid, x, y, glyph);
	
	// Clamp coordinates to valid range instead of rejecting them
	// UnNetHack seems to have a coordinate calculation bug that generates
	// values slightly over 80. We'll clamp them to prevent crashes.
	int clamped_x = x;
	int clamped_y = y;
	...
	if (x != clamped_x || y != clamped_y) {
		debuglog("CLAMPED coordinates from %dx%d to %dx%d", x, y, clamped_x, clamped_y);
	}
	...
	if(glyph == NO_GLYPH)
		...
		tile = -1;  // Invalid glyph, use default
	else
		tile = glyph2tile[glyph];
	glyph_t ch;
	int col;
	unsigned int special;
	// Use clamped coordinates to prevent array access violations
	mapglyph(glyph, &ch, &col, &special, clamped_x, clamped_y);
	int uni = get_unicode_codepoint(ch);
	debuglog("calling JNI: tile=%d uni=%d col=%d special=%d", tile, uni, col, special);
	JNICallV(jPrintTile, wid, clamped_x, clamped_y, tile, uni, nhcolor_to_RGB(col), special);
	debuglog("JNI call completed");
}
