	...
	switch (ptr->mlet) {
		...
		if (mm == PM_EXECUTIONER) {
			...
			case PM_SOLDIER:
			  ...
			  } else w1 = rn2(2) ? SPEAR : SHORT_P_SWORD;
			  break;
			case PM_SERGEANT:
			  w1 = rn2(2) ? FLAIL : MACE;
			  break;
			...
			case 0:
			    ...
			    if (rn2(3)) (void)mongets(mtmp, ELVEN_SHORT_P_SWORD);
		...
		if (mm == PM_HOBBIT) {
		    ...
		    if (!rn2(4)) {
			(void)mongets(mtmp, DWARVISH_SHORT_P_SWORD);
...
# endif
		...
		switch (mm != PM_ORC_CAPTAIN ? mm :
			...
			if(!rn2(3)) (void)mongets(mtmp, ORCISH_SHORT_P_SWORD);
		...
		if (!rn2(4))
			(void)mongets(mtmp, (rn2(3) ? KNIFE : SHORT_P_SWORD));
		break;
	    case S_LIZARD:
		if (mm == PM_SALAMANDER)
 ...
 */
boolean
propagate(int mndx, BOOLEAN_P tally, BOOLEAN_P ghostly)
{
	boolean result;
