...
static int gbuf_start[ROWNO];
static int gbuf_stop[ROWNO];
    ...
    int x, y, glyph;
{
    ...
    /* CRITICAL FIX: Validate coordinates before ANY use to prevent array overruns */
    if (x < 0 || x >= COLNO || y < 0 || y >= ROWNO) {
	/* Silently ignore out-of-bounds coordinates - UnNetHack bug generates these */
	return;
    }
...
flush_screen(int cursor_on_u)
{
	...
	if (gbuf_start[y] > gbuf_stop[y]) continue; // Skip if no valid range
	
	// CRITICAL FIX: Use clamped values for loop, don't modify originals
	start_x = gbuf_start[y];
	stop_x = gbuf_stop[y];
	
	if (start_x < 0) start_x = 0;
	if (start_x >= COLNO) start_x = COLNO - 1;
	if (stop_x < 0) stop_x = 0;
	if (stop_x >= COLNO) stop_x = COLNO - 1;
	
	// Skip if still invalid after clamping
	if (start_x > stop_x) continue;
	
	for (x = start_x; x <= stop_x; x++) {
	    gbuf_entry *gptr = &gbuf[y][x];
	    if (gptr->new) {
