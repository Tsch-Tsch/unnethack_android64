...
STATIC_OVL void
check_here(BOOLEAN_P picked_some)
{
...
lootcont:
	...
	for (cobj = level.objects[cc.x][cc.y]; cobj; cobj = nobj) {
		...
		if (cobj->olocked) {
			...
			if(flags.autounlock && cobj->otyp == IRON_SAFE && (otmp = carrying(STETHOSCOPE))) {
			    pick_lock(otmp, cc.x, cc.y, TRUE);
			} else if(flags.autounlock &&
...
#ifdef ANDROID
			else if (flags.autokick && cobj->otyp != IRON_SAFE) {
		    	doforce_ext(TRUE);
			}
#endif
		    }
 ...
 */ 
int
dump_container(struct obj* container, BOOLEAN_P destroy_after)
{
	struct obj* otmp,*otmp2;
	int ret = 0;
	schar ltyp;
